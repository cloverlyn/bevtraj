...
from md_scenarionet.common_utils import read_scenario, read_dataset_summary
...
class BaseDataset(Dataset):
    ...
    def load_data(self):
        ...
        for cnt, data_path in enumerate(self.data_path):
            ...
            self.cache_path = os.path.join(data_path, f'cache_{self.config.model_name}')
    ...
    def preprocess(self, scenario):
        ...
        if self.config['multisample_per_scene']: 
            skip = self.config['frame_skip_interval']
            max_start_index = track_length - total_steps
            start_indices = list(range(0, max_start_index + 1, skip))
        else:
            start_indices = [0]
        ...
        for start_index in start_indices:
            ...
            for k, v in tracks.items():
                ...
                all_state = [state['position'], state['length'], state['width'], state['height'], state['heading'],
                            state['velocity'], state['valid']] # type, x,y,z,l,w,h,heading,vx,vy,valid
                ...
                all_state = all_state[start_index : start_index + total_steps]
            ...
            if 'timestamp_ns' in ret:
                ret['timestamp_ns'] = ret['timestamp_ns'][start_index : start_index + total_steps]
                ret['curr_timestamp_ns'] = ret['timestamp_ns'][ret['current_time_index']]
    ...
    def process(self, preprocessed_list):
        ret_list = []
        for info in preprocessed_list:
            ...
            ret_dict = {
                ...
                'center_objects_world': center_objects.copy(),
                ...
                'obj_trajs_future_state': obj_trajs_future_state, #[batch_size, N, pred_time_step, (x,y,vx,vy)]
                'obj_trajs_future_mask': obj_trajs_future_mask, #[batch_size, N, pred_time_step]
                'center_gt_trajs': center_gt_trajs,
                'center_gt_trajs_mask': center_gt_trajs_mask,
                'center_gt_final_valid_idx': center_gt_final_valid_idx,
                'center_gt_trajs_src': obj_trajs_full[track_index_to_predict]
            }
            if 'curr_timestamp_ns' in info:
                curr_timestamp_ns = info['curr_timestamp_ns']
                ret_dict['curr_timestamp_ns'] = np.array([curr_timestamp_ns] * len(track_index_to_predict))
            ...
            for i in range(sample_num):
                ...
                for k, v in ret_dict.items():
                    ret_dict_i[k] = v[i]
                sample_list.append(ret_dict_i)
                
            ret_list += sample_list
        return ret_list
    ...
    def ego_get_map_data(self, ego_objects, center_objects, map_infos):
        num_center_objects = center_objects.shape[0]
        ego_objects = np.expand_dims(ego_objects, axis=0).repeat(num_center_objects, axis=0) # (num_center_objects, 10)
        
        
        def transform_center_agents_to_ego_coordinates(center, ego):
            """
            This function transforms the center agents data to the ego vehicle's coordinates.
            """
            center[:, 0:3] -= ego[:, 0:3]
            center[:, 6] -= ego[:, 6]
            center = center[:, None, :]
            center[:, :, 0:2] = common_utils.rotate_points_along_z(
                points= center[:, :, 0:2],
                angle= -ego[:, 6]
            )
        ...
        def transform_to_local_coordinates(neighboring_polylines, local_objects):
            neighboring_polylines[:, :, 0:3] -= local_objects[:, None, 0:3]
            neighboring_polylines[:, :, 0:2] = common_utils.rotate_points_along_z(
                points=neighboring_polylines[:, :, 0:2],
                angle=-local_objects[:, 6]
            )
            neighboring_polylines[:, :, 3:5] = common_utils.rotate_points_along_z(
                points=neighboring_polylines[:, :, 3:5],
                angle=-local_objects[:, 6]
        ...
        polylines = np.expand_dims(map_infos['all_polylines'].copy(), axis=0).repeat(num_center_objects, axis=0) # (num_center_objects, num_polylines, 7)

        map_polylines = transform_to_local_coordinates(polylines, ego_objects)
        num_of_src_polylines = self.config['max_num_roads']
        map_infos['polyline_transformed'] = map_polylines
        ...
        #MODIFY
        e2c = transform_center_agents_to_ego_coordinates(center_objects, ego_objects) #(num_center_objects, 1, 10)
        center_heading = e2c[:, 0, 6]
        
        #map_polylines translate & rotate
        b, m, n, d = map_polylines.shape
        map_polylines = np.reshape(map_polylines, (b, m* n, d))
        map_polylines[:, :, 0:3] -= e2c[:, :, 0:3]
        map_polylines[:, :, 0:2] = common_utils.rotate_points_along_z(
            points=map_polylines[:, :, 0:2],
            angle=-center_heading
        )
        map_polylines = np.reshape(map_polylines, (b, m, n, d))
        
        
        #map_polylines_center translate & rotate
        map_polylines_center[:, :, 0:3] -= e2c[:, :, 0:3]
        map_polylines_center[:, :, 0:2] = common_utils.rotate_points_along_z(
                points=map_polylines_center[:, :, 0:2],
                angle=-center_heading
            )
